...
# 配置参数
LINE_POSITION = 1/2  # 检测线位置（画面高度比例）
...
class VehicleTracker:
    ...
    def update(self, detections, line_y):
        ...
        for det in detections:
            ...
            if best_match is not None:
                ...
                self._create_new_track(class_name, bbox, centroid, line_y, confidence)
                current_ids.append(self.track_id)

        # 更新计数并清理丢失的目标
        self._update_counts(line_y)
    ...
    def _create_new_track(self, class_name, bbox, centroid, line_y, confidence):
        self.track_id += 1
    ...
    def _update_counts(self, line_y):
        for track_id, track in self.tracks.items():
            if track['counted']:
                continue
            ...
            if 'initial_frames' in track and len(track['initial_frames']) == INITIAL_FRAMES:
                top_count = sum(1 for c in track['initial_frames'] if c[1] < line_y)
                track['initial_side'] = 'top' if top_count > INITIAL_FRAMES // 2 else 'bottom'
            ...
            first_y = track['positions'][0][1]
            last_y = track['positions'][-1][1]

            if first_y > line_y and last_y < line_y:  # 从下到上穿越 → in
                self.count_history['in'][track['class']] += 1
                track['counted'] = True
            elif first_y < line_y and last_y > line_y:  # 从上到下穿越 → out
...
def process_video(video_path, model, class_names, class_colors):
    ...
    try:
        while cap.isOpened():
            ...
            tracker.update(detections, line_y)
...
def main():
    # 加载 YOLO 模型
    model = YOLO(r"ultralytics/runs/detect/yolo11x/weights/best.pt")
    ...
    # 定义要处理的视频文件列表
    video_files = ["ultralytics/data/F_weekday_1900/DJI_20250220191524_0001_V.MP4",
                   "ultralytics/data/F_weekday_1900/DJI_20250220191548_0002_V.MP4",
                   "ultralytics/data/F_weekday_1900/DJI_20250220192249_0003_V.MP4",
                   "ultralytics/data/F_weekday_1900/DJI_20250220192950_0004_V.MP4",
                   "ultralytics/data/F_weekday_1900/DJI_20250220193650_0005_V.MP4",
                   "ultralytics/data/F_weekday_1900/DJI_20250220194351_0006_V.MP4",
                   "ultralytics/data/F_weekday_1900/DJI_20250220194534_0007_V.MP4"]
