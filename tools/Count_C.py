    ...
    def _update_counts(self, width, height, offset):
        for track_id, track in self.tracks.items():
            ...
            if 'initial_frames' in track and len(track['initial_frames']) == INITIAL_FRAMES:
                left_bottom_count = sum(1 for c in track['initial_frames'] if c[1] > c[0] * (height / width) + offset)
                track['initial_side'] = 'left_bottom' if left_bottom_count > INITIAL_FRAMES // 2 else 'right_top'
            ...
            first_x, first_y = track['positions'][0]
            last_x, last_y = track['positions'][-1]

            first_side = 'left_bottom' if first_y > first_x * (height / width) + offset else 'right_top'
            last_side = 'left_bottom' if last_y > last_x * (height / width) + offset else 'right_top'

            if first_side == 'left_bottom' and last_side == 'right_top':  # 从左下方穿到右上方 → out
                self.count_history['out'][track['class']] += 1
                track['counted'] = True
            elif first_side == 'right_top' and last_side == 'left_bottom':  # 从右上方穿到左下方 → in
...
def process_video(video_path, model, class_names, class_colors):
    ...
    try:
        while cap.isOpened():
            ...
            tracker.update(detections, width, height, offset)
...
def main():
    ...
    # 定义要处理的视频文件列表
    video_files = ["ultralytics/data/D_20250301_weekend_1039-1140/DJI_20250301104756_0001_V.MP4",
                   "ultralytics/data/D_20250301_weekend_1039-1140/DJI_20250301105457_0002_V.MP4",
                   "ultralytics/data/D_20250301_weekend_1039-1140/DJI_20250301110158_0003_V.MP4",
                   "ultralytics/data/D_20250301_weekend_1039-1140/DJI_20250301110858_0004_V.MP4"]
