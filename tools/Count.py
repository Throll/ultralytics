...
# 配置参数
LINE_POSITION = 1.7/3  # 检测线位置（画面宽度比例）
...
class VehicleTracker:
    ...
    def update(self, detections, line_x):
        ...
        for det in detections:
            ...
            if best_match is not None:
                ...
                self._create_new_track(class_name, bbox, centroid, line_x, confidence)
                current_ids.append(self.track_id)

        # 更新计数并清理丢失的目标
        self._update_counts(line_x)
    ...
    def _create_new_track(self, class_name, bbox, centroid, line_x, confidence):
        self.track_id += 1
    ...
    def _update_counts(self, line_x):
        for track_id, track in self.tracks.items():
            if track['counted']:
                continue
            ...
            if 'initial_frames' in track and len(track['initial_frames']) == INITIAL_FRAMES:
                left_count = sum(1 for c in track['initial_frames'] if c[0] < line_x)
                track['initial_side'] = 'left' if left_count > INITIAL_FRAMES // 2 else 'right'
            ...
            first_x = track['positions'][0][0]
            last_x = track['positions'][-1][0]

            if first_x < line_x and last_x > line_x:  # 从左到右穿越 → in
                self.count_history['in'][track['class']] += 1
                track['counted'] = True
            elif first_x > line_x and last_x < line_x:  # 从右到左穿越 → out
...
def parse_detections(frame, model):
    ...
    for result in results:
        ...
        for box in boxes:
            ...
            class_name = model.names[class_id]  # 使用模型的 names 属性获取类别名称
...
def process_video(video_path, model, class_names, class_colors):
    ...
    try:
        while cap.isOpened():
            ...
            tracker.update(detections, line_x)
...
def main():
    # 加载 YOLO 模型
    model = YOLO("/hdd/sa/ultralytics/runs/detect/yolo11x/weights/best.pt")
    ...
    # 定义要处理的视频文件列表
    video_files = ["ultralytics/data/E_weekday_1200-1400/DJI_202503181223_001/DJI_20250318122445_0001_V.MP4",
                   "ultralytics/data/E_weekday_1200-1400/DJI_202503181223_001/DJI_20250318123145_0002_V.MP4",
                   "ultralytics/data/E_weekday_1200-1400/DJI_202503181223_001/DJI_20250318123846_0003_V.MP4",
                   "ultralytics/data/E_weekday_1200-1400/DJI_202503181223_001/DJI_20250318124546_0004_V.MP4",
                   "ultralytics/data/E_weekday_1200-1400/DJI_202503181223_001/DJI_20250318125247_0005_V.MP4",
                   "ultralytics/data/E_weekday_1200-1400/DJI_202503181254_002/DJI_20250318125520_0001_V.MP4",
                   "ultralytics/data/E_weekday_1200-1400/DJI_202503181254_002/DJI_20250318130220_0002_V.MP4",
                   "ultralytics/data/E_weekday_1200-1400/DJI_202503181254_002/DJI_20250318130921_0003_V.MP4",
                   "ultralytics/data/E_weekday_1200-1400/DJI_202503181254_002/DJI_20250318131622_0004_V.MP4",
                   "ultralytics/data/E_weekday_1200-1400/DJI_202503181254_002/DJI_20250318132322_0005_V.MP4",
                   "ultralytics/data/E_weekday_1200-1400/DJI_202503181326_003/DJI_20250318132710_0001_V.MP4",
                   "ultralytics/data/E_weekday_1200-1400/DJI_202503181326_003/DJI_20250318133411_0002_V.MP4",
                   "ultralytics/data/E_weekday_1200-1400/DJI_202503181326_003/DJI_20250318134111_0003_V.MP4",
                   "ultralytics/data/E_weekday_1200-1400/DJI_202503181326_003/DJI_20250318134812_0004_V.MP4",
                   "ultralytics/data/E_weekday_1200-1400/DJI_202503181326_003/DJI_20250318135512_0005_V.MP4"]
    # 初始化总的计数历史字典
    ...
    for class_name in class_names:
        ...
        print(f"{class_name}: IN {total_count_history['in'][class_name]} ({in_rate:.2f} 个/h) | OUT {total_count_history['out'][class_name]} ({out_rate:.2f} 个/h)")
